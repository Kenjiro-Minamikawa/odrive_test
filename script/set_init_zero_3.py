...
def prepare_targets_from_current_positions(
    ...
    # 最終位置をもう一度送る
    for node_id in cmd_pos:
        init_pos = current_pos[node_id] + offset[node_id]
        send_set_input_pos(
            node_id,
        ...
        latest[node_id]["initial_pos_rev"] = init_pos
        ...
        if now - last_input >= INPUT_INTERVAL:
            ...
            for node_id in NODE_IDS:
                if node_id in {1}:
                    ...
                    wave = 0.4 * chirp_signal[count % chirp_signal.shape[0]]
                else:
                    wave = 0
            ...
            print_counter += 1
            count += 1
            if print_counter % PRINT_PER_STEP == 0:
                print_counter = 0
                print_table()
